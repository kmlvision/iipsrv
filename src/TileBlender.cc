...
void TileBlender::getRawTilesAndPreprocess(Session* session, int resolution, int tile, const std::vector<BlendingSetting> &blending_settings)
{
  ...
  for (int i = 0; i < session->images.size(); ++i) {
    ...
    if (rawtile.compressionType != UNCOMPRESSED) {
      throw string(
              logging_prefix + "rawtile.compressionType -> retrieved image data already compressed, uncompressed data buffer required");
    }
...
void TileBlender::getRawRegionsAndPreprocess(Session* session, const std::vector<BlendingSetting> &blending_settings)
{
  ...
  if( session->loglevel >= 3 ){
    *(session->logfile) << logging_prefix + "Using resolution " << requested_res << " with size " << im_width << "x" << im_height << endl;
  }
  ...
  if( session->loglevel >= 3 ){
    *(session->logfile) << logging_prefix + "Requested scaled region size is " << resampled_width << "x" << resampled_height
  ...
  for (int i = 0; i < session->images.size(); ++i) {
    ...
    if (raw_region.compressionType != UNCOMPRESSED) {
      throw string(
              logging_prefix + "raw_region.compressionType -> retrieved image data already compressed, uncompressed data buffer required");
    }
...
//      }
    ...
    if( (session->view->getViewWidth()!=resampled_width) || (session->view->getViewHeight()!=resampled_height) ){
    ...
    this->raw_tiles.push_back(raw_region);  // raw_region added to raw_tiles vector, tiles that are all ready to be blended
  } // end foreach image
}


void TileBlender::blendTiles(Session* session, int resolution, int tile, const std::vector<BlendingSetting> &blending_settings)
{
...
void TileBlender::blendRegions(Session* session, const std::vector<BlendingSetting> &blending_settings)
{
  ...
  RawTile blended_tile(0, tmp.resolution, tmp.hSequence, tmp.vSequence, tmp.width, tmp.height, 3, 8);  // 3 channels (RGB) and 8bit
  blended_tile.dataLength = blended_tile.width * blended_tile.height * out_channels;
  uint8_t *dst = new uint8_t[blended_tile.dataLength]();
  blended_tile.data = dst;  // this is cleaned up by raw tile

...
// blend settings are passe in as part of the argument string "&BlendJSON"
// BlendJSON defines a serialized JSON string with the blending settings,
// we just split the string. The path to the file may optionally have an extension.
void IIIFBlend::run(Session *session, const string &src) {
    ...
    if (src.find("&") == std::string::npos) {
        session->response->setError("2 0", src);
        throw string("IIIFBlend: check argument string to contain all data");
    }

    // get json string by splitting the input into IIIF params and the json string
    std::string iiif_params(src.substr(0, src.find("&")));
    std::string json_string(src.substr(src.find("&") + 1, src.length()));
    if (session->loglevel >= 4) {
        (*session->logfile) << "IIFBlend :: IIIF params:\n" << iiif_params << "\n" << endl;
        (*session->logfile) << "IIIFBlend :: JSON string:\n" << json_string << "\n" << endl;
    }

    // Various string variables
    string suffix, cmd_filename_prefix, file_ext, params;

    // First filter and decode our the IIIF URL part (we parse the json later)
    URL url(iiif_params);
    ...
    if (lastSlashPos != string::npos) {
        ...
        if (suffix.substr(0, 4) == "info") {
            cmd_filename_prefix = argument.substr(0, lastSlashPos);
        } else {
            ...
            cmd_filename_prefix = argument.substr(0, positionTmp);
            size_t ext_delimiter_idx = cmd_filename_prefix.find_last_of(".");
            if (ext_delimiter_idx != string::npos) {
            ...
            if (session->loglevel >= 4) {
                (*session->logfile) << "IIIFBlend :: cmd_filename_prefix: " << cmd_filename_prefix << endl;
                (*session->logfile) << "IIIFBlend :: file extension: " << file_ext << "\n" << endl;
            }
        }
    } else {
    ...
    if (suffix == "info.json") {
        ...
        if (host.length() > 0) {
            ...
            if (request_uri.empty()) {
                throw invalid_argument(
                        "IIIF: REQUEST_URI was not set in FastCGI request, so the ID parameter cannot be set.");
            }
        ...
        string iiif_id = session->headers["HTTP_X_IIIF_ID"].empty() ? escapedFilename
                                                                    : session->headers["HTTP_X_IIIF_ID"];
        ...
        if (izer.hasMoreTokens()) {
            ...
            if (session->loglevel >= 4) {
                *(session->logfile) << "IIIFBlend :: Requested Size: " << requested_width << "x" << requested_height
                                    << endl;
            }
        ...
        if (izer.hasMoreTokens()) {
            ...
            if (!(rotation == 0 || rotation == 90 || rotation == 180 || rotation == 270 || rotation == 360)) {
                throw invalid_argument(
                        "IIIFBlend: invalid argument: currently implemented rotation angles are 0, 90, 180 and 270 degrees");
            }
        ...
        if (izer.hasMoreTokens()) {
            ...
            if (quality == "native" || quality == "color" || quality == "default") {
                ...
                throw invalid_argument(
                        "IIIFBlend: unsupported quality parameter - must be one of native, color or grey");
            }

            numOfTokens++;

            if (session->loglevel >= 4) {
                *(session->logfile) << "IIIFBlend :: Requested Quality: " << quality << " with format: " << extension
                                    << endl;
            }
        }
    ...
    if (session->loglevel >= 3) {
        if (suffix == "info.json") {
            *(session->logfile) << "IIIFBlend :: " << suffix << " request for " << (*session->image)->getImagePath()
                                << endl;
        } else {
            *(session->logfile) << "IIIFBlend :: image request for " << (*session->image)->getImagePath()
                                << " with arguments: region: " << session->view->getViewLeft() << ","
                                << session->view->getViewTop() << ","
